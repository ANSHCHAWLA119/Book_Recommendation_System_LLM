import streamlit as st     # python framewrok for creating user interface
from langchain_google_genai import ChatGoogleGenerativeAI   # langchain framework
...
'''
Its an interface to help the user so that he can pick a book of his choice . So i first created a button which generate the top 100 books (popular) from geminis knowledge Base.
Now from these 100 books we will choose top 10 best books (popular) to narrow down which are the best books . So if user finds any book which is best by its title so he 
will choose that book according to the genre. Now he will get all the description regarding that book like title,year,description it depends on llm what it will generate regarding that book .
'''